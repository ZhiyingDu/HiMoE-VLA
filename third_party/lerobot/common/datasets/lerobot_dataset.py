...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def load_hf_dataset(self) -> datasets.Dataset:
        ...
        if self.episodes is None:
            ...
            if "ZhiyingDu_Pi0_Dataset" in path:
                cache_dir = path.replace("ZhiyingDu_Pi0_Dataset", "ZhiyingDu_Pi0_Dataset_cache")
            else:
...
class MultipleDatasetWeightedDistributedBatchSampler(torch.utils.data.BatchSampler):
    ...
    def prepare_sample_size(self):
        # 原始每个子数据集的样本数
        self._dataset_lengths = [len(dataset) for dataset in self.dataset._datasets]
        ...
        # ===== 打印采样统计 =====
        total_raw = sum(self._dataset_lengths)

        print("\n===== Dataset Sampling Info =====")
        print(f"{'Idx':<3} {'Name':<40} {'RawLen':>10} {'RawRatio':>10} {'Weight':>10} {'SampleSize':>12} {'SampleRatio':>12}")
        print("-" * 100)

        for i, dataset in enumerate(self.dataset._datasets):
            name = getattr(dataset, "repo_id", f"dataset_{i}")
            raw_len = self._dataset_lengths[i]
            raw_ratio = raw_len / total_raw if total_raw > 0 else 0
            weight = self.weights[i]
            sample_size = self._sample_size[i]
            sample_ratio = sample_size / self.total_size if self.total_size > 0 else 0

            print(f"{i:<3} {name:<40} {raw_len:>10} {raw_ratio:>9.2%} {weight:>10.3f} {sample_size:>12} {sample_ratio:>11.2%}")

        print("-" * 100)
        print(f"Total raw length: {total_raw}")
        print(f"Total weighted sample size: {self.total_size}")
        print(f"Iter size (batch_size * num_replicas): {iter_size}")
        print(f"Num iters: {self.num_iters}\n")

    def create_indices(self, dataset_id, epoch):
        indices = list(range(self._dataset_lengths[dataset_id]))
        if self.shuffle:
            g = torch.Generator()
            g.manual_seed((self.seed + epoch) * len(self.dataset._datasets) + dataset_id)
    ...
    def shuffle_dataset_indices(self, indices):
        ...
        dataset_index_list = []
        dataset_count = [0] * len(self.dataset._datasets)
    ...
    def prepare_indices(self):
        indices = []
        for i in range(len(self.dataset._datasets)):
    ...
    def __iter__(self):
        ...
        # print("Samples:", dataset_index_list[:200])
